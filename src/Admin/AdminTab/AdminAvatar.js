...
import AvtarHandle from '../AdminSubFIle/AvtarHandle';
...
const AdminAvatar = ({ colors }) => {
    ...
    const handleSaveAvatar = () => {
        ...
        if (editorRef.current) {
            ...
            const resizedBase64 = resizedCanvas.toDataURL('image/png');
            setCropUrl(resizedBase64);
            sendBase64ToAPI(resizedBase64);
            setLoading(false);
            setError(null);
        }
    };

    const sendBase64ToAPI = (base64Data) => {
        fetch(`${config.apiUrl}/admins/defaultAvatar`, {
            credentials: 'include',
            method: 'POST',
            headers: {
            ...
            .then((response) => {
                ...
                setError(null);
                message.success({
                    content: 'Successfully Upload Default Avatar',
                    duration: 5,
                    onClose: handleReset
                });
            })
    ...
    return (
        <>
            <h2 className='card p-2 mb-4 w-25 text-center mx-auto'>Default Avatar Upload</h2>
